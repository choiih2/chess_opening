...
<<<<<<< HEAD
=======
**힌트는 두 종류를 구분해 보여줍니다.** 연습 중 "힌트 화살표"를 켜면 내 차례마다
보드에 화살표가 뜹니다.

| 색 | 뜻 | 판정 |
|---|---|---|
| 황동색 | 메인라인 | 그 국면에서 가장 많이 둔 수 |
| 초록색 | 탁월 수 | 아래 세 조건을 모두 만족하는 수 |

탁월 수 조건은 표본 30판 이상, 승률 50% 이상, 그리고 메인라인보다 승률이 4%p 이상
높을 것입니다. 조건을 만족하는 수가 없으면 초록 화살표는 나오지 않고 "뚜렷하게 나은
수는 없는 국면"이라고 표시합니다. 즉 초록 화살표는 **엔진이 판단한 최선수가 아니라
실전에서 성적이 더 좋은 수**입니다. 정석보다 잘 알려지지 않았지만 실제로는 이득을
보는 수를 찾아 주는 쪽에 가깝습니다.

오답을 두면 나오는 흰색 화살표는 내 레퍼토리에 등록된 수를 알려주는 것으로,
힌트와는 별개입니다.
...
- **Stockfish 평가(5단계)**
=======
- **Stockfish 평가(5단계)** — 엔진 기준의 진짜 최선수와 전술적 묘수를 짚으려면
  이게 필요합니다. 현재 탁월 수는 통계 기반이라 "많은 사람이 실수하는 수"는
  잡아내지만 "한 수 짜리 전술"은 놓칩니다.
>>>>>>> 1c4ad35ce796730ee570be28e85def78e24e8f26
